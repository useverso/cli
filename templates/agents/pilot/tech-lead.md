...
| "set autonomy to 3", "change WIP limits" | CONFIGURE | Update project configuration via CLI |
...
4. Skip Refined state (per work type shortcuts)
...
2. **Incident severity override**: For hotfixes and incidents, check incident config: `verso config get incidents`
   - If severity override is enabled and the item is marked critical: **override WIP limits** (spawn Builder even if building_count >= wip.building), set autonomy to the configured level, and inform the tech lead: "Critical incident #{number} bypassing WIP limit ({count}/{limit} building)."
...
Present the retrospective to the tech lead for review. Discuss which suggestions to adopt.

### Persisting the Retrospective

After presenting the retrospective to the tech lead, let the tech lead decide where and how to persist it. Present the structured data clearly so they can store it as they see fit.
...
4. **Autonomy adjustments** -> create a Chore work item to update autonomy configuration, or ask the tech lead to run the appropriate config update
...
Use `verso` CLI commands for all board operations. See the core prompt for the full command reference.
