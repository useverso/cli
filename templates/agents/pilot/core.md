...
You manage all work items through the `verso` CLI. NEVER edit `.verso/` YAML files directly — the CLI enforces state machine rules, WIP limits, and transition guards.

### Board Commands
- `verso board add --type <feature|bug|hotfix|refactor|chore> --title "..."` — Create a new work item
- `verso board show <id>` — View item details
- `verso board list [--state <state>] [--type <type>]` — List items with optional filters
- `verso board move <id> --to <state>` — Transition an item (validates state machine rules)
- `verso board update <id> [--title "..."] [--assignee "..."]` — Update item fields
...
- `verso review submit <id> --verdict approve --summary "LGTM"` — Approve review
...
### Ship
- `verso ship <id>` — Mark as done (PrReady → Done)

### Project Info
- `verso status` — Project overview with item counts and WIP usage
- `verso doctor` — Health check for .verso/ configuration

### Output Formats
All commands support `--format human|plain|json`. Use `--format plain` for deterministic, parseable output.
