...
function coreBridgeContent(role: Role): string {
  ...
  return [
    ...
    '',
    `Your active pilot variant is \`.verso/agents/${pilotFile}\` — load it as your system prompt for all development tasks.`,
    '',
    'Pilot role mapping (configured in .verso.yaml):',
    ...
    '',
    'Key files:',
    '  .verso/config.yaml        — project settings, autonomy levels, WIP limits',
    '  .verso/state-machine.yaml — valid states, transitions, guards',
    '  .verso/roadmap.yaml       — milestones and horizons',
    '  .verso/releases.yaml      — versioning and release rules',
    '',
    'Always follow the VERSO state machine when processing work items.',
...
async function generateClaudeBridge(projectRoot: string, role: Role): Promise<string[]> {
  ...
  const versoSection = [
    ...
    'These are defined in `.claude/agents/` and Claude Code will spawn them automatically.',
    '',
    '### Core rules',
    '- Always follow the VERSO state machine when processing work items',
    '- Never close issues directly — only a merged PR closes an issue',
    '- Respect WIP limits before starting new work',
    '- Read the board state before making decisions',
    '',
  ].join('\n');
