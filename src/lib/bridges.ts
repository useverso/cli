...
function coreBridgeContent(role: Role): string {
  ...
  return [
    ...
    '### Role switching',
    '',
    'When work items need implementation, switch to a builder role (follow `.verso/agents/builder.md`).',
    'When PRs need review, switch to a reviewer role (follow `.verso/agents/reviewer.md`).',
    '',
    '### Core rules',
    '',
...
async function generateClaudeBridge(projectRoot: string, role: Role): Promise<string[]> {
  ...
  const versoSection = [
    ...
    'Claude Code will spawn them automatically when you use the Task tool.',
    '',
    '### Core rules',
    '',
    '- Create issues in Captured state IMMEDIATELY when the user describes work — capture first, refine later',
